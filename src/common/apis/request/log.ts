...
import { isClientEnv, isAbsoluteUrl, isDevEnv, trimUrlSlashes } from "@/common/utils"
...
export const requestLog = (option: IRequestLogOptions) => {
    ...
    // 当前为开发环境 或者 客户端环境且开启调试模式时输出请求日志
    if (isDevEnv() || isClientEnv() && isEnableDebug()) {
        ...
        if (!isAbsoluteUrl(url)) {
            ...
            url = `${process.env.NEXT_PUBLIC_API_BASE_PATH}${url && `/${url}`}`
